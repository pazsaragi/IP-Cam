...
All cgi commands need authentication before they can be executed. 
    ...
    http://192.168.1.88/cgi-bin/hi3510/param.cgi?cmd=getvencattr&-chn=11&-usr=admin
    &-pwd=admin
    http://192.168.1.6/cgi-bin/hi3510/param.cgi?cmd=getwirelessattr&-usr=admin&-pwd=
    admin
"""
...
class PTZ_commands(object):
    ...
    def __init__(self, user, pwd, IP, port=554):
        ...
        self.usage = "/cgi-bin/hi3510/param.cgi?cmd=ptzctrl"
        self.step = "&-step="
        self.speed = "&-speed="
        self.Commands = {}
    ...
    def pan(self, Direction, Steps, Speed):
        ...
        comm = self.base_url + self.ptzctrl + self.step + Steps + self.Commands[direction] + self.speed + Speed + self.auth
        print(comm, direction)
        move = requests.get(comm)
        return move
    ...
    def setmottoattr(self, panspeed, tiltspeed, 
        ...
        """
        comm = self.base_url + self.Commands['GETMOTOATTR'] + str(panspeed) + str(tiltspeed) + str(panscan) + str(tiltscan) \
         + movehome + ptzalarmmask + str(alarmpresetindex) + self.auth
        print(comm)
    ...
    def setMotion(self, enable, s, window, x, y, w, h):
        #/cgi-bin/hi3510/param.cgi?cmd=setmdattr&-enable=1
        # &-s=50&-name=1&-x=0&-y=0&-w=60&-h=60
        """
